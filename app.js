...
const contractAddress = '0xaf2f356ff4bffb9bbc53390dff9435f0f7a2bd5d';//'0x30289f4a54bfae52be1bfa939b5672eb1504b6ad';
const contractABI = [
//     {
//         "inputs": [],
...
//             {
//                 "indexed": false,
//                 "internalType": "enum CoachAndMentor.SessionType",
//                 "name": "sessionType",
//                 "type": "uint8"
//             },
//             {
...
//                 "indexed": false,
//                 "internalType": "uint256",
//                 "name": "sessionTypeIndex",
//                 "type": "uint256"
//             }
//         ],
//         "name": "SessionTypeOutOfRange",
//         "type": "event"
//     },
//     {
...
//                     }
//                 ],
//                 "internalType": "struct CoachAndMentor.Booking[]",
//                 "name": "",
//                 "type": "tuple[]"
//             }
//         ],
...
async function bookSession() {
    ...
    if (isNaN(sessionType) || sessionType < 0 || sessionType > 3) {
        displayMessage('Invalid session type. Please enter a number between 0 and 3.', 'error');
        return;
    }
...
async function checkBalance() {
    ...
    try {
        ...
        displayMessage('Contract balance: ' + ethers.utils.formatEther(balance) + ' ETH', 'info');
    } catch (error) {
...
async function getUserBookings() {
    ...
    try {
        ...
        displayMessage('Failed to retrieve user bookings: ' + error.message, 'error');
    }
}

document.getElementById('getUserBookings').addEventListener('click', getUserBookings);
...
async function getAllBookings() {
    ...
    try {
        ...
        const isOwner = userAddress === await contract.owner();
        if (!isOwner) {
            displayMessage('Only the contract owner can access all bookings.', 'error');
            return;
        }
        ...
        displayMessage('Failed to retrieve all bookings: ' + error.message, 'error');
    }
}

document.getElementById('getAllBookings').addEventListener('click', getAllBookings);
...
function displayBookings(title, bookings) {
    ...
    bookings.forEach(booking => {
        html += `<li>User: ${booking.user}, Session Type: ${booking.sessionType}, Amount: ${ethers.utils.formatEther(booking.amount)} ETH</li>`;
    });
    html += '</ul>';
